...
#[cfg(unix)]
fn write_mock_llm_client(config_dir: &Path, port: u16) -> Result<()> {
    let clients_dir = config_dir.join("clients");
...
fn retry_all_fail_shows_warnings_in_tui() -> Result<()> {
    ...
    session.tmux.send_text(&format!(
        "{} -a {RETRY_AGENT_NAME} --session || echo HARNX_EXIT:$?",
