//! Abort signal used to propagate Ctrl-C / Ctrl-D cancellation across
//! long-running operations. Pure (no terminal or UI deps). The
//! terminal-polling helper that reads key events to set the signal lives
//! in `harnx/src/utils/abort_signal.rs` because it uses `crossterm`.
