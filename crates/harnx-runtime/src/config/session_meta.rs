...
mod tests {
    ...
    fn picker_label_includes_unread_marker_when_unread() {
        assert_picker_marker(true, "● ", "unread session label should start with marker");
    }

    #[test]
    fn picker_label_no_marker_when_read() {
        assert_picker_marker(
            false,
            "test-session",
            "read session label should not have marker",
        );
    }

    /// Parameterized helper for picker label marker tests.
    fn assert_picker_marker(unread: bool, prefix: &str, reason: &str) {
        let mut session = session_meta("test-session");
        session.title = Some("My Session".to_string());
        session.unread = unread;

        let label = session.picker_label();
        assert!(label.starts_with(prefix), "{}: got: {label}", reason);
    }
}
