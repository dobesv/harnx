...
struct SettingCompletion<'a> {
    name: &'a str,
    value_prefix: &'a str,
}

impl Config {
    fn complete_first_arg(
        &self,
        cmd: &str,
    ...
    fn complete_use_tools_arg(&self, arg: &str) -> Vec<String> {
        let mut prefix = String::new();
        let mut ignores = HashSet::new();
        if let Some((v, _)) = arg.rsplit_once(',') {
        ...
        values.extend(
            self.tool_declarations_for_use_tools(Some("*"), self.active_package().as_deref())
                .0
                .iter()
                .map(|v| v.name.clone()),
        );
    ...
    fn complete_set_candidates(&self, setting: SettingCompletion<'_>) -> Vec<String> {
        match setting.name {
            ...
            "use_tools" => self.complete_use_tools_arg(setting.value_prefix),
    ...
    fn complete_use_tool(&self) -> Vec<(String, Option<String>)> {
    ...
    fn complete_drop_tool(&self) -> Vec<(String, Option<String>)> {
        let agent = self.extract_agent();
        let current = agent.use_tools().unwrap_or_default();
        current.into_iter().map(|s| (s, None)).collect()
    }
    ...
    pub fn command_complete(
        ...
        let values = match (cmd, args.len()) {
            (_, 1) => self.complete_first_arg(cmd, precomputed_agents),
            (".set", 2) => self
                .complete_set_candidates(SettingCompletion {
                    name: args[0],
                    value_prefix: args[1],
                })
                .into_iter()
                .map(|v| (v, None))
                .collect(),
            (".use", 2) if args[0] == "tool" => self.complete_use_tool(),
            (".drop", 2) if args[0] == "tool" => self.complete_drop_tool(),
            (".agent", _) => complete_agent_variables(args[0]),
            _ => vec![],
        };
...
mod tests {
    ...
    #[test]
    fn set_completion_uses_setting_name_and_value_prefix() {
        let config = Config::default();
        let expected = (!config.stream).to_string();
        assert_eq!(
            config.command_complete(".set", &["stream", &expected], vec![]),
            vec![(expected, None)]
        );
        assert!(config
            .command_complete(".set", &["stream", "not-a-boolean"], vec![])
            .is_empty());
    }
