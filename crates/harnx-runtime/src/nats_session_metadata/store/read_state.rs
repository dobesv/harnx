...
impl SessionMetadataStore {
    ...
    /// Bumps attention for an agent-scoped storage key to at least `seq`, idempotent.
    ///
    /// Publishes read-invalidation if the stored value changes.
    pub async fn bump_attention(&self, storage_key: &str, seq: u64) -> Result<()> {
        self.patch_read_state(storage_key, |state| {
    ...
    /// Marks a storage key as read: advances its cursor to attention and clears manual unread.
    ///
    /// Publishes read-invalidation if the stored value changes.
    pub async fn mark_read(&self, storage_key: &str) -> Result<()> {
        self.patch_read_state(storage_key, |state| {
    ...
    /// Marks a storage key as manually unread without touching its cursor.
    ///
    /// Publishes read-invalidation if the stored value changes.
    pub async fn mark_unread(&self, storage_key: &str) -> Result<()> {
        self.patch_read_state(storage_key, |state| {
    ...
    async fn patch_read_state<F>(&self, storage_key: &str, mut patch: F) -> Result<()>
    where
        F: FnMut(&mut SessionReadState) -> Result<bool>,
    {
        ...
        for attempt in 0..CAS_RETRY_LIMIT {
            ...
            match self.write_read_state(&key, &state, revision).await {
                Ok(new_revision) => {
                    self.publish_read_invalidation(storage_key, new_revision)
                        .await;
                    return Ok(());
                }
    ...
    async fn publish_read_invalidation(&self, storage_key: &str, revision: u64) {
        ...
        {
            log::warn!(
                "failed to publish session read-state invalidation: storage_key={storage_key} error={error:#}"
            );
        }
    }
}
