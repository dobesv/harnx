...
use harnx_runtime::remote_session_cleanup::{
    run_periodic_remote_cleanup_with, CleanupOutcome, GC_LAST_RUN_EPOCH_HOUR_KEY,
};
...
async fn worker_collects_stale_session_without_interactive_client() -> Result<()> {
    ...
    let storage_key = metadata.storage_key();
    metadata_store.create(&metadata).await?;
    let stale_activity = SessionActivity {
        first_activation_at: None,
        last_activity_at: Utc.timestamp_opt(1, 0).single().expect("valid timestamp"),
    };
    metadata_store
        ...
        .put(
            activity_key(&storage_key),
            serde_json::to_vec(&stale_activity)?.into(),
        )
        .await?;
    ...
    journal
        .record(
            &ToolRequest {
                ...
                parent_session_id: Some(storage_key.clone()),
...
async fn worker_remote_session_cleanup_deduplicates_by_epoch_hour() -> Result<()> {
    ...
    let first_outcome =
        run_periodic_remote_cleanup_with(&cleanup_config, 1, "local", gc_session_id, epoch_hour)
            .await;
...
async fn worker_remote_session_cleanup_marker_read_failure_releases_lease() -> Result<()> {
    ...
    let bootstrap =
        run_periodic_remote_cleanup_with(&cleanup_config, 1, "local", gc_session_id, 300_000).await;
    ...
    let failed =
        run_periodic_remote_cleanup_with(&cleanup_config, 1, "local", gc_session_id, 300_001).await;
