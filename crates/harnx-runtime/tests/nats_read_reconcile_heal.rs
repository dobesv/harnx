...
async fn missed_invalidation_heals_via_refetch() -> Result<()> {
    ...
    let cached_state: SessionReadState = {
        ...
        store_a.get_read_state(&storage_key(&session_id)).await?
    };
    ...
    let state_after_reconcile = store_a.get_read_state(&storage_key(&session_id)).await?;
    assert!(
...
async fn multiple_missed_invalidations_heal_on_reconnect() -> Result<()> {
    ...
    let cached_a = store_a.get_read_state(&storage_key(&session_id)).await?;
    ...
    let state_after_reconnect = store_a.get_read_state(&storage_key(&session_id)).await?;
    assert!(
    ...
    store_b
        .bump_attention(&storage_key(&session_id), 20)
        .await?;
    let _ = tokio::time::timeout(Duration::from_secs(2), read_sub_a_reconnected.next()).await;

    let state_final = store_a.get_read_state(&storage_key(&session_id)).await?;
...
async fn lost_attention_bump_heals_via_log_reconciliation() -> Result<()> {
    ...
    let state_before = store.get_read_state(&storage_key(&session_id)).await?;
    assert!(
    ...
    // Verify repair
    let state_after = store.get_read_state(&storage_key(&session_id)).await?;
    assert!(
...
async fn cross_client_activity_converges_on_reconcile() -> Result<()> {
    ...
    let mut cached_a = store_a.get_read_state(&storage_key(&session_id)).await?;
    let mut cached_b = store_b.get_read_state(&storage_key(&session_id)).await?;
    assert!(cached_a.is_unread());
    ...
    // Both refetch
    cached_a = store_a.get_read_state(&storage_key(&session_id)).await?;
    cached_b = store_b.get_read_state(&storage_key(&session_id)).await?;
    assert!(!cached_a.is_unread());
    assert!(!cached_b.is_unread());
...
async fn two_clients_converge_to_last_writer_state() -> Result<()> {
    ...
    let state_a1 = store_a.get_read_state(&storage_key(&session_id)).await?;
    let state_b1 = store_b.get_read_state(&storage_key(&session_id)).await?;
    ...
    store_a.mark_read(&storage_key(&session_id)).await?;
    let state_a2 = store_a.get_read_state(&storage_key(&session_id)).await?;
    ...
    let state_a3 = store_a.get_read_state(&storage_key(&session_id)).await?;
    let state_b3 = store_b.get_read_state(&storage_key(&session_id)).await?;
