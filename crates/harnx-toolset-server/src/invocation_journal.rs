...
/// across incoming requests so busy traffic cannot starve reconciliation.
    ...
    futures_util::stream::unfold(js, move |js| async move {
        ...
        let result = async {
            let execution = js
                ...
                .await?;
            harnx_nats_common::registry::reconcile_bucket_replicas(
                &js,
                BUCKET,
                execution.cached_info().config.num_replicas,
            )
            .await
        }
        .await;
