...
`harnx-bash-tools` is a native toolset server providing controlled shell command execution (`exec` and `spawn`) bounded by sandbox rules and filesystem roots.

## Overview

The server runs in native toolset mode by default and supports `--mcp-stdio` for backward compatibility. Command execution is restricted to allowed filesystem roots and extra path allowlists. If no roots or allowlists are configured, command execution and working directory resolution are denied by default.

## Installation

To install `harnx-bash-tools` from the `harnx` workspace:
