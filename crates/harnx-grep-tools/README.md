...
Native Rust toolset server exposing a single `grep_query` tool that searches GitHub code via the grep.app API.
...
```bash
cargo install --path crates/harnx-grep-tools
```
...
Run the server directly from a `tool_servers/grep.yaml` configuration:

```yaml
command: harnx-grep-tools
args: []
```
