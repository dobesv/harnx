...
const TOKEN: &str = "mcp-bridge-sigterm-test-token";
...
async fn sigterm_removes_the_bridge_registration() -> Result<()> {
    ...
    let child = Command::new(env!("CARGO_BIN_EXE_harnx-mcp-bridge"))
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("spawn harnx-mcp-bridge")?;
    let pid = child.id();
    let mut bridge = BridgeHandle(child);
    ...
    // terminated. Before wiring `serve_with_shutdown`, the bridge had no
    // shutdown token any signal handler could cancel.
    let killed = unsafe { libc::kill(pid as libc::pid_t, libc::SIGTERM) };
    assert_eq!(killed, 0, "failed to send SIGTERM to harnx-mcp-bridge");
