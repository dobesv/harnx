...
async fn nats_remote_agent_turn_runs_on_shared_worker_and_persists_bare_agent() -> Result<()> {
    ...
    let run_id = match prompt(&handle, "complete this remote turn").await {
        crate::session_actor::PromptResult::Accepted { run_id } => run_id,
        other => anyhow::bail!("remote prompt was not accepted: {other:?}"),
    };
    tokio::time::timeout(CI_SAFE_TIMEOUT, async {
        loop {
            match events
                .recv()
                .await
                .context("remote turn event stream closed")?
            {
                Event::RunFinished(event) if event.run_id.to_string() == run_id => {
                    return Ok::<_, anyhow::Error>(());
                }
                Event::RunError(error) => anyhow::bail!("remote turn failed: {}", error.message),
                _ => {}
            }
        }
    })
    .await
    .context("remote turn did not finish")??;

    let resumed_run_id = match prompt(&handle, "resume this remote session").await {
        crate::session_actor::PromptResult::Accepted { run_id } => run_id,
        other => anyhow::bail!("remote resume was not accepted: {other:?}"),
    };
    tokio::time::timeout(CI_SAFE_TIMEOUT, async {
        loop {
            match events
                .recv()
                .await
                .context("remote resume event stream closed")?
            {
                Event::RunFinished(event) if event.run_id.to_string() == resumed_run_id => {
                    return Ok::<_, anyhow::Error>(());
                }
                Event::RunError(error) => anyhow::bail!("remote resume failed: {}", error.message),
                _ => {}
            }
        }
    })
    .await
    .context("remote resume did not finish")??;
