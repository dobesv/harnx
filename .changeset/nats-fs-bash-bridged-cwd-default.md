...
feat(nats): migrate `fs` and `bash` MCP servers to run bridged over NATS via `harnx-mcp-bridge`. Add `--default-root-cwd` to `harnx-mcp-fs` and `harnx-mcp-bash` to seed allowed roots from process CWD with `$HOME`-ancestor protection. Export ambient `HARNX_PACKAGE_DIR` to NATS tool servers so bundled hooks resolve when wrapped in `harnx-mcp-hooks-proxy`.
