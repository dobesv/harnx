...
# @arg bins*[harnx|harnx-serve|harnx-acp-server|harnx-mcp-bash|harnx-mcp-bash-sandbox-run|harnx-mcp-fs|harnx-mcp-time|harnx-mcp-plans|harnx-aws-creds|harnx-pkg|harnx-proxy-auth] Restrict the install to one or more bins (default: all)
install() {
    bins=("${argc_bins[@]}")
    if [[ ${#bins[@]} -eq 0 ]]; then
        bins=(harnx harnx-serve harnx-acp-server harnx-mcp-bash harnx-mcp-fs harnx-mcp-time harnx-mcp-plans harnx-mcp-bash-sandbox-run harnx-aws-creds harnx-pkg harnx-proxy-auth)
    fi
...
test-init-config() {
    ...
    mkdir -p "$HARNX_CONFIG_DIR"
    config_file="$HARNX_CONFIG_DIR/config.yaml"
...
# @env HARNX_PROVIDER!
# @env HARNX_CONFIG_DIR=tmp/test-provider-env
# @arg args~
test-no-config() {
    mkdir -p "$HARNX_CONFIG_DIR"
    rm -rf "$HARNX_CONFIG_DIR/config.yaml"
    cargo run -- "$@"
}
