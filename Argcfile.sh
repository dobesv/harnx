...
# @arg bins*[harnx|harnx-serve|harnx-acp-server|harnx-mcp-bash|harnx-mcp-bash-sandbox-run|harnx-mcp-fs|harnx-mcp-time|harnx-mcp-plans|harnx-aws-creds|harnx-proxy-auth] Restrict the install to one or more bins (default: all)
install() {
    bins=("${argc_bins[@]}")
    if [[ ${#bins[@]} -eq 0 ]]; then
        bins=(harnx harnx-serve harnx-acp-server harnx-mcp-bash harnx-mcp-fs harnx-mcp-time harnx-mcp-plans harnx-mcp-bash-sandbox-run harnx-aws-creds harnx-proxy-auth)
    fi
