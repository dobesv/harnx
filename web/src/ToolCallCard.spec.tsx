...
describe('ToolCallCard', () => {
  ...
  describe('sub-agent call rendering and default expansion', () => {
    ...
    it('detects sub_agent marker in a serialized JSON-string result and uses sub-agent presentation', () => {
      const props = {
        toolName: 'custom_dispatch',
        toolCallId: 'call_serialized_marker',
        result: JSON.stringify({
          response: 'Serialized child response.',
          sub_agent: { agent: 'researcher', session_id: 'sess-serialized' },
        }),
        status: { type: 'complete' },
      } as any;
      ...
      // Serialized marker must be detected: card expands by default and uses the
      // sub-agent 'Response:' label rather than the generic 'Result:'.
      const header = container.querySelector('.aui-tool-call-header');
      expect(header).toHaveAttribute('aria-expanded', 'true');
      expect(screen.getByText('Response:')).toBeInTheDocument();
      expect(screen.queryByText('Result:')).toBeNull();
      expect(screen.getByText('Serialized child response.')).toBeInTheDocument();
    });

    it('detects sub_agent_progress marker in a serialized JSON-string result and uses sub-agent presentation', () => {
      const props = {
        toolName: 'custom_dispatch',
        toolCallId: 'call_serialized_progress',
        result: JSON.stringify({
          response: 'Progress-only child response.',
          sub_agent_progress: { status: 'done' },
        }),
        status: { type: 'complete' },
      } as any;
