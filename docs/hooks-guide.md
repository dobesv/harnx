...
* **`Ask` Outcome & Headless Limitation**: A `PreToolUse` hook returning `Ask` surfaces as a tool confirmation request (`ToolApprovalRequiredError`). The worker routes the prompt to the attached frontend's session-scoped confirmation subject via NATS. Direct turns and queued continuation turns both use this route. If the frontend detaches or the route shuts down, the request fails closed (deny). In headless worker environments without an attached frontend, requests fail closed immediately.
