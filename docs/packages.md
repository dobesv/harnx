...
Package clients inherit model limits, prices, capabilities, and request patches
from the shared `crates/harnx/models.yaml` catalog. Name the block you want with
`model_catalog:`, which works for any client type and any filename:
...
Omit it and the filename decides instead: a client's type picks its own block,
`codex` borrows OpenAI's, and an `openai-compatible` client matches a provider
by filename prefix, so `bedrock.yaml` and `bedrock-us.yaml` both inherit Bedrock
even when qualified as `my-pkg/bedrock`. That fallback predates `model_catalog`
and remains only for compatibility; prefer naming the catalog, since the
filename rule quietly gives `deepseek-proxy.yaml` the DeepSeek catalog and
quietly denies one to `aws-prod.yaml`.

Explicit `models:` entries override catalog entries of the same name, replacing
them outright rather than merging field by field — an entry that omits
`input_price` has no price at all. They are unnecessary for models already in
the catalog.

When adding package model defaults, update shared catalog metadata rather than
copying it into each package. Reasoning aliases generated by
`scripts/model_variants.py` also need their generation rule updated, or a catalog
refresh can replace a hand-edited alias. The shipped-agent model test exercises
package namespacing, metadata inheritance, and single-provider fallback chains.
...
agents:
  ...
  - 'if .name == "coder" then .model_fallbacks = ["openai:gpt-4o"] end'
