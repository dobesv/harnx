...
from the shared `crates/harnx/models.yaml` catalog. `codex` inherits the OpenAI
catalog. For `openai-compatible`, the bare client filename selects the provider
by prefix: both `bedrock.yaml` and `bedrock-us.yaml` inherit Bedrock models even
when qualified as `my-pkg/bedrock`. Explicit `models:` entries override catalog
entries of the same name; they are unnecessary for models already in the catalog.
