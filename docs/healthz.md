...
- **NATS-consumer servers** (`harnx-bash-tools`, `harnx-fs-tools`, `harnx-exa-tools`, `harnx-grep-tools`, `harnx-k8s-sandbox-tools`, `harnx-time-tools`, `harnx-claude-compatible-hook-server`): Become ready after NATS subscriptions connect and initial queue requests are flushed. The Kubernetes sandbox gateway waits until all three of its toolset registrations are visible.
