...
- **HARNX_METRICS_ADDR**: Environment variable fallback for `--metrics-addr` on shared-entrypoint binaries (`harnx-bash-tools`, `harnx-fs-tools`, `harnx-exa-tools`, `harnx-fetch-tools`, `harnx-grep-tools`, `harnx-k8s-sandbox-tools`, `harnx-time-tools`, `harnx-plans-tools`, `harnx-claude-compatible-hook-server`, `harnx-mcp-remote`, `harnx-mcp-bridge`). Address format is `IP:PORT` or `:PORT` (blank host binds `0.0.0.0`).
...
- `HARNX_NATS_URL` — NATS server or cluster URL. `nats://` and `tls://` use the
  ...
  what traverses an HTTP load balancer. Front-ends ignore this variable for `__local__`
  routing; use `HARNX_NATS_SERVER` to join an external cluster.
- `HARNX_NATS_TOKEN` — token auth for that connection. Front-ends ignore this variable
  for `__local__` routing; use `HARNX_NATS_SERVER` to join an external cluster.
- `HARNX_NATS_SERVER` — names a cluster from `nats_servers/<name>.yaml` for front-ends (`harnx`, `harnx-serve`). Runs the front-end as a pure client without starting a local `nats-server`, worker, or tool servers. Bare agent names route to that cluster as `<agent>@<name>`. Local (`__local__`) agents are unavailable in this mode. When unset, front-ends self-host a local broker and worker and ignore operator `HARNX_NATS_URL`/`HARNX_NATS_TOKEN` for `__local__` routing.
