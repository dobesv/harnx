...
- **use_tools**: Specify which tools to make available (YAML list or comma-separated string). Supports glob patterns (`*`, `{a,b}` brace expansion).
...
- **use_tools**: Specify which tools this agent can use (YAML list or comma-separated string). Supports glob patterns (`*`, `{a,b}` brace expansion).
