...
- **No cost attribution**: Token usage numbers are recorded on LLM request spans, but tracing itself does not attach financial cost to spans. Cumulative token usage and dollar cost metrics are available via the Prometheus endpoint (see [Prometheus Metrics](metrics.md)).
